...
class GANDiscriminator(nn.Module):
    def __init__(self, n_channels=3, gpu=False):
        ...
        self.init_conv = nn.Conv2d(self.n_channels, 16, 4, 2, 1, bias=False)
...
class GANGenerator(nn.Module):
    def __init__(self, n_z, n_channels=3, preprocess=None, gpu=False):
        ...
        self.outconv = nn.ConvTranspose2d(16, self.n_channels, 4, 2, 1, bias=False)
    ...
    def forward(self, x):
        if not torch.is_tensor(x):
            ...
            x = torch.FloatTensor(x)
        if self.gpu and not x.is_cuda:
            # print("Put GAN gen input on cuda")
            x = x.cuda()
