...
if __name__ == '__main__':
    ...
    if TASK == 'imagenet':
        PLOT_INFO = [
            ...
            ('BAPP_result/attack_multi_imagenet_AE9408_.log', 'b-', 'QEBA-AE9408'),
            ('BAPP_result/attack_multi_imagenet_AE128_.log', 'b--', 'QEBA-AE128'),
            ('BAPP_result/attack_multi_imagenet_oldAE9408_.log', 'r', 'QEBA-oldAE9408'),
            ('BAPP_result/attack_multi_imagenet_VAE9408_.log', 'g--', 'QEBA-VAE'),
            ('BAPP_result/attack_multi_imagenet_GAN128_.log', 'y-', 'QEBA-GAN128'),
            ('BAPP_result/attack_multi_imagenet_GAN9408_.log', 'y--', 'QEBA-GAN9408'),
        ...
        PLOT_INFO = [
            ...
            # ('BAPP_result/attack_multi_celeba_resize9408_.log', 'g', 'QEBA-S9408'),
            # ('BAPP_result/attack_multi_celeba_DCT9408_.log', 'b', 'QEBA-F9408'),
            # # ('BAPP_result/attack_multi_celeba_PCA9408basis_.log', 'r-', 'QEBA-I9408'),
            # ('BAPP_result/attack_multi_celeba_PCA9408_.log', 'm', 'QEBA-I9408'),
            # # ('BAPP_result/attack_multi_celeba_AE128_.log', 'b-', 'QEBA-AE128'),
            # ('BAPP_result/attack_multi_celeba_AE9408_.log', 'c', 'QEBA-AE9408'),
            # ('BAPP_result/attack_multi_celeba_VAE9408_.log', 'c-.', 'QEBA-VAE9408'),
            # ('BAPP_result/attack_multi_celeba_GAN128_.log', 'r', 'QEBA-GAN128'),
            # ('BAPP_result/attack_multi_celeba_GAN9408_.log', 'r-.', 'QEBA-GAN9408'),
    ...
    for logs, info in zip(all_logs, PLOT_INFO):
        ...
        for i in range(len(logs)):
            if i % 2 == 0:
                pgen1_logs.append(logs[i])
            else:
                pgen2_logs.append(logs[i])

        print()
        print(pgen1_logs)
        print(np.array(pgen1_logs).shape)
        print()
        print(pgen2_logs)
        print(np.array(pgen2_logs).shape)
        print()

        new_logs = np.concatenate((np.array(pgen1_logs), np.array(pgen2_logs)), axis=1)
        print(new_logs.shape)

        avg_X = []
        avg_y = []
        for t in range(len(logs[0])):
            avg_X.append(mean([log[t][0] for log in new_logs]))
            avg_y.append(mean([log[t][1] for log in new_logs], logarithm=True))
    ...
    plt.savefig('./plots/multi_var_%s_mean.pdf'%TASK, bbox_inches='tight')
    # assert 0
    ...
    for logs, info in zip(all_logs, PLOT_INFO):
        ...
        for t in range(len(logs[0])):
            avg_X.append(mean([log[t][0] for log in new_logs]))
            avg_y.append(mean([log[t][stat_id+1] for log in new_logs]))
            #avg_y.append(mean([log[t][1] for log in logs], logarithm=False))
    ...
    plt.savefig('./plots/multi_var_%s_%s.pdf'%(TASK,ATTR_NAME[stat_id]), bbox_inches='tight')
    assert 0
    ...
    plt.savefig('./plots/multi_var_%s_success.pdf'%TASK, bbox_inches='tight')
    assert 0
    ...
    #            plt.yscale('log')
    #        plt.legend()
    #fig.savefig('multi_var_%s_%s-%s.pdf'%(TASK, img_st, img_st+N_img-1))
    #assert 0
